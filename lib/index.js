...
var bezierCurve$1 = require('@jiaminghi/bezier-curve');
...
function transformColor(reverse) {
  ...
  return function (color$1) {
    ...
    if (isString && reverse) return color$1;
    if (isArray && !reverse) return __spreadArrays(color$1);
    if (isString && !reverse) return color.getRgbaValue(color$1);
    if (isArray && reverse) return color.getColorFromRgbValue(color$1);
...
var bezierCurve = {
  ...
  draw: function draw(_a, _b) {
    ...
    if (!cache.points || cache.points.toString() !== points.toString()) {
      var hoverPoints = bezierCurve$1.bezierCurveToPolyline(points, 20);
      Object.assign(cache, {
...
var smoothline = {
  ...
  draw: function draw(_a, _b) {
    ...
    if (!cache.points || cache.points.toString() !== points.toString()) {
      var bezierCurve_1 = bezierCurve$1.polylineToBezierCurve(points, close);
      var hoverPoints = bezierCurve$1.bezierCurveToPolyline(bezierCurve_1);
      Object.assign(cache, {
        points: deepClone(points),
